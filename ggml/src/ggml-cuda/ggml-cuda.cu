...
static bool ggml_backend_cuda_buffer_cpy_tensor(ggml_backend_buffer_t buffer, const ggml_tensor * src, ggml_tensor * dst) {
    if (ggml_backend_buffer_is_cuda(src->buffer)) {
        ...
        if (src_physical == dst_physical) {
            ...
            return false;
#else
            // host-staged fallback for multi-GPU without peer access (turbo)
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    // TQ weight types (turboquant model weights) use a fused dp4a path (all batch sizes),
    // not the generic mmvq/mmq kernels. Handles ne[1]=1 (decode) and ne[1]<=8 (multi-token /
    // speculative decoding) via warp-shuffle WHT; large prefill runtime-converts TQ4_1S -> q8_0
    // scratch and uses cuBLAS for tensor-core throughput without permanent 1.7x VRAM cost.
    // (This function is the per-device handler, so turbo's original !split guard is implicit.)
    const bool is_tq_weight = src0->type == GGML_TYPE_TQ4_1S || src0->type == GGML_TYPE_TQ3_1S;
    if (is_tq_weight) {
        if (ne11 <= MMVQ_MAX_BATCH_SIZE) {
...
static bool ggml_backend_cuda_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_SET_ROWS:
            {
                // turbo KV-cache types require head_dim divisible by their group size
