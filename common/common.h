...
struct common_params_speculative_draft {
    ...
    // [graft: turbo] DFlash/EAGLE3 bool flags. STAP3 drives speculative decoding via the
    // `types` enum vector above; TheTom/llama-cpp-turboquant's server+models layer additionally
    // gates the DFlash/EAGLE3 engines on these booleans (see tools/server/server-context.cpp:
    // `speculative.draft.dflash || speculative.draft.eagle3`). Kept alongside STAP3's enum so
    // BOTH code paths resolve. Set together in common/arg.cpp when COMMON_SPECULATIVE_TYPE_DRAFT_*.
...
struct common_params {
    ...
    // [graft: turbo] mmap bools. The merged src/llama-model-loader is a blend: STAP3 body
    // reads `load_mode`, turbo body reads `use_mmap`/`use_direct_io`/`use_mlock`. Keep BOTH so
    // neither code path fails to compile. common_params_to_context_params maps load_mode->ctx.
    ...
    // [graft: turbo] slot cache-key params. tools/server/server-context.cpp reads
    // `slot_cache_key_similarity` + `slot_cache_key_min_prefix` (slot_cache_key_* refs).
